...
def get_network_data_based_on_user(user_address):
    ...
    sql_query = f"""
       select 
        ...
        sum(amount) as value
        from axelar.core.EZ_SQUID
        WHERE  destination_chain in ('ethereum', 'avalanche', 'binance', 'arbitrum', 'polygon', 'celo', 'fantom', 'moonbeam') and  sender in ( '{user_address}')
        group by 1,2
    """
    return get_result_from_query(sql_query)
...
def get_source_chain_based_on_user_address(user_address):
...
def get_destination_chain_based_on_user_address(user_address):
